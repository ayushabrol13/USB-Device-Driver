...
static ssize_t nr_read(struct file *filp, char *buffer, size_t count, loff_t *ppos){
	...
	dev = filp->private_data;								//recover our data pointer from the open file structure (saved inside
															// the open() function )
	rs=copy_to_user(buffer, dev->testbuff, dev->lenbuff);	//Copy a block of data into user space. 
															//	Returns number of bytes that could not be copied.
															//	On success, this will be zero. 
															//	copy_to_user(to, from, nbdata);
	printk("_NR_ return of the reading:%d\n", (unsigned int)rs);

	*ppos += dev->lenbuff-rs;								//Whatever the amount of data the method transfers, it should generally 															//	update the file position at *ppos to represent the current file position 																//	after successful completion of the system call. The kernel then 															//	propagates the file position change back into the file structure when 																//	appropriate.
	return dev->lenbuff-rs;									//See the return value in the comments at the beginning of the function
}
